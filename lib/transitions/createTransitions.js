...
module.exports = function createTransitions(animation, stateFrom, stateTo, stateNames) {
  var paths = [];
  var pathAnimations;
  ...
  // the prop to animate
  getPathsToProperties(stateFrom, stateTo, paths, null, stateNames);
...
function getPathsToProperties(stateFrom, stateTo, paths, keys, stateNames) {
  ...
  if (!stateFrom && !stateTo){
    throw new Error("'" + stateNames[0] + "' to '" + stateNames[1] + "' transition could not be executed. State definitions cannot be found.");
  }
  else if(!stateFrom){
    throw new Error("'" + stateNames[0] + "' to '" + stateNames[1] + "' transition could not be executed. Starting state definition cannot be found.");
  }
  else if (!stateTo){
    throw new Error("'" + stateNames[0] + "' to '" + stateNames[1] + "' transition could not be executed. Ending state definition cannot be found.");
  }
  else if(stateFrom){
    for(var i in stateFrom) {
      ...
      if(stateTo){
        ...
        if (!stateFrom && stateTo){
          throw new Error("State '" + stateNames[0] + "' defined in attempted transition not found.");
        }
        else if (stateFrom && !stateTo){
          throw new Error("State '" + stateNames[1] + "' defined in attempted transition not found.");
        }
        else {
          throw new Error("States '" + stateNames[0] + "' and " + stateNames[1] + " defined in attempted transition not found.");
        }
      }
    }
  }
...
function percentTweenFunction(obj) {
  ...
  return function(time,start,end) {
    return tween(time,Number(start.slice(0,-1)),Number(end.slice(0,-1)))+'%';
  }
};